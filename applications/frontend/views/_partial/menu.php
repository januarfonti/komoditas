...
<?php
	foreach ($menu as $parent => $parent_params)
	{
		...
		elseif(!empty($parent_params['anak']))
		{
			$parent_active = ( $ctrler==$parent ) ? 'active' : '';
			$parent_name = $parent_params['name'];
			$parent_url = $parent_params['url'];
			...
			foreach ($parent_params['anak'] as $name => $url)
			{
